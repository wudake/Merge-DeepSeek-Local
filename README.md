...
# 可选: 在 .env 中设置 OPENAI_API_KEY (使用 OpenAI 转写引擎时需要)
# 默认使用本地 Whisper 模型，无需 API Key
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `JWT_SECRET` | `app-shared-jwt-secret-...` | JWT 签名密钥 (生产环境务必修改) |
...
| `OPENAI_API_KEY` | (空) | OpenAI API Key (使用 OpenAI 转写时需要) |
